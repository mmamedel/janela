...
| `chmod +x` on `@scriptc/llvm-linux-x64-gnu`'s helper binary in CI (`.github/workflows/ci.yml`) | The 0.1.3 optional package ships `bin/scriptc-llvm-codegen` without its executable bit surviving pnpm's content-addressable store; scriptc's own `SC3003` readable+executable check (`native-codegen.js:118-121`) then rejects it, and no lifecycle script (scriptc's `postinstall` skips native invocation entirely on hosts its runtime pack supports — `scripts/warm-cache.mjs`) restores it | scriptc ships the helper package with its executable bit preserved, or chmods it in `postinstall` — see `scratchpad/issue-0.1.3-linux-execbit.md` |
| `SCRIPTC_LINKER=clang` override for Windows CI/local builds (`.github/workflows/ci.yml`, docs/windows-notes.md, packages/janela/README.md) | scriptc 0.1.3's new default executable-link route hardcodes `zig` as `windows-x64-msvc`'s linker (`WINDOWS_X64_MSVC_TARGET.defaultLinker` in `backend/targets.js`); `SCRIPTC_CC=clang` doesn't avoid it (that only selects the deprecated legacy pipeline). Worse, zig's own link then fails rebuilding its bundled libc++ against janela's llvm-mingw-`clang++`-built webview shim object — an undiagnosable failure, since scriptc's error truncation (`index.js:198-205`) drops everything but the trailing 40 stderr lines. `SCRIPTC_LINKER` overrides `defaultLinker` (`backend/linker.js:20-21`), so CI pins it back to `clang` | scriptc offers a non-zig default linker route for `windows-x64-msvc` that is compatible with a separately-compiled C++ object, or documents/fixes the zig-libc++-vs-mingw-libstdc++ mismatch — see `scratchpad/issue-0.1.3-windows-zig.md` |
