...
| `zig` install step for Windows CI/local builds (`.github/workflows/ci.yml`, docs/windows-notes.md, packages/janela/README.md) | scriptc 0.1.3's new default executable-link route hardcodes `zig` as `windows-x64-msvc`'s linker (`WINDOWS_X64_MSVC_TARGET.defaultLinker` in `backend/targets.js`) to link its zig-mingw-built runtime pack against the LLVM helper's object; `SCRIPTC_CC=clang` no longer avoids it (that only selects the deprecated legacy pipeline) | scriptc documents/vendors this requirement itself (e.g. an optional `zig` npm helper package, matching its LLVM helper packages), or offers a non-zig linker route for `windows-x64-msvc` — see `scratchpad/issue-0.1.3-windows-zig.md` |
