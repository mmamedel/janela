...
clang **and** `zig` on Windows (see [Windows](#windows) below). iOS
additionally needs Xcode and `zig`; Android needs a JDK, the Android SDK, the
NDK and `zig`. A worked example lives in [`examples/demo`](examples/demo) —
...
**You also need a `zig` binary on `PATH`.** scriptc 0.1.3 links Windows
executables through its precompiled runtime pack (built against zig's mingw
sysroot) rather than compiling one; the windows-x64-msvc target hardcodes
`zig` as its linker, so `scriptc build`/`janela build` fail with `spawn zig
ENOENT` without it. See [docs/windows-notes.md](../../docs/windows-notes.md)
for detail.
